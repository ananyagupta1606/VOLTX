...
export default function ProductDetail({ product, onClose, onAddToCart }) {
  ...
  return (
    ...
    >
      ...
      >
        {/* Hero image */}
        <div className="aspect-video bg-gradient-to-br from-gray-800 to-gray-700 flex items-center justify-center text-8xl rounded-t-2xl">
          {p.emoji}
        </div>
        ...
        <div className="p-6">
          ...
          <div className="flex justify-between items-start gap-4 mb-2">
            <div>
              <p className="text-cyan-400 text-xs font-bold uppercase tracking-widest mb-1">{p.brand}</p>
              <h2 className="text-white font-extrabold text-2xl leading-tight">{p.name}</h2>
            </div>
            <button
              onClick={onClose}
          ...
          <div className="mb-4"><Stars n={p.rating} /></div>
          <p className="text-gray-400 text-sm leading-relaxed mb-5">{p.desc}</p>

          {/* Specs grid */}
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 mb-5">
            {p.specs.split(" · ").map(s => (
              <div key={s} className="bg-gray-800 border border-gray-700 rounded-xl p-3">
                <p className="text-gray-500 text-xs uppercase tracking-wider mb-1">Spec</p>
                <p className="text-white font-mono text-sm">{s}</p>
              </div>
            ))}
            <div className="bg-gray-800 border border-gray-700 rounded-xl p-3">
              <p className="text-gray-500 text-xs uppercase tracking-wider mb-1">Category</p>
              <p className="text-white font-mono text-sm">{p.cat}</p>
            </div>
            <div className="bg-gray-800 border border-gray-700 rounded-xl p-3">
              <p className="text-gray-500 text-xs uppercase tracking-wider mb-1">In Stock</p>
              <p className="text-green-400 font-mono text-sm">✓ Available</p>
            </div>
          </div>
