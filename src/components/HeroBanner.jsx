      ...
      <div className="relative max-w-7xl mx-auto px-6 py-24 md:py-32 flex flex-col md:flex-row items-center gap-12">
        ...
        <div className="flex-1 text-center md:text-left">
          ...
          <p className="text-gray-400 text-lg max-w-lg mx-auto md:mx-0 leading-relaxed mb-8">
            ...
            curated for the tech-obsessed. Free shipping on every order.
          </p>
          ...
          <div className="flex flex-wrap gap-5 mt-10 justify-center md:justify-start">
            {[
              ...
              <div key={b.label} className="flex items-center gap-2 text-gray-500 text-xs font-medium">
                <span>{b.icon}</span>
                <span>{b.label}</span>
              </div>
            ))}
        ...
        <div className="flex-shrink-0 relative w-72 h-72 md:w-80 md:h-80 hidden md:block">
          ...
          {/* Orbiting cards */}
          {[
            { emoji: "💻", name: "MacBook Pro", price: "$3,499", top: "0%",   left: "50%",  translate: "-translate-x-1/2" },
            { emoji: "📱", name: "iPhone 16",   price: "$1,299", top: "50%",  left: "90%",  translate: "-translate-y-1/2" },
            { emoji: "🎧", name: "Sony XM5",    price: "$279",   top: "85%",  left: "60%",  translate: "-translate-x-1/2" },
            { emoji: "🖥️", name: "RTX 5090",    price: "$1,999", top: "50%",  left: "5%",   translate: "-translate-y-1/2" },
          ].map(c => (
            <div
              key={c.name}
              className={`absolute ${c.translate} bg-gray-800/90 border border-gray-700 rounded-2xl px-3 py-2.5 backdrop-blur flex items-center gap-2.5 shadow-xl hover:border-cyan-400/50 hover:scale-105 transition-all cursor-pointer`}
              style={{ top: c.top, left: c.left }}
            >
              <span className="text-2xl">{c.emoji}</span>
              <div>
                <p className="text-white text-xs font-semibold leading-tight">{c.name}</p>
                <p className="text-cyan-400 text-xs font-bold">{c.price}</p>
              </div>
            </div>
          ))}
