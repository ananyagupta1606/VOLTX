export default function CartSidebar({ cart, onClose, onUpdateQty, onCheckout }) {
      ...
      <div className="fixed right-0 top-0 bottom-0 w-96 max-w-[95vw] bg-gray-900 border-l border-gray-700 z-50 flex flex-col">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-700">
          <h2 className="text-white font-extrabold text-lg">
            🛒 Your Cart
            <span className="ml-2 text-gray-400 font-normal text-sm">({count} item{count !== 1 ? "s" : ""})</span>
          </h2>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-3">
          {cart.length === 0 ? (
            ...
            cart.map(item => (
              <div key={item.id} className="bg-gray-800 border border-gray-700 rounded-xl p-3 flex gap-3">
                {/* Emoji */}
                <div className="w-12 h-12 rounded-lg bg-gray-900 flex items-center justify-center text-2xl flex-shrink-0">
                  {item.emoji}
                </div>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-cyan-400 font-mono text-xs mt-0.5">${item.price.toLocaleString()}</p>
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    ...
                    >−</button>
                    <span className="text-white font-mono text-sm min-w-[20px] text-center">{item.qty}</span>
                    <button
                      onClick={() => onUpdateQty(item.id, 1)}
                      className="w-6 h-6 rounded bg-gray-700 border border-gray-600 text-gray-300 flex items-center justify-center text-sm hover:border-cyan-400 hover:text-cyan-400 transition-colors"
                    >+</button>
                ...
                <div className="flex flex-col items-end justify-between flex-shrink-0">
                  <span className="text-white font-semibold text-sm">${(item.price * item.qty).toLocaleString()}</span>
                  <button
        ...
        {cart.length > 0 && (
          <div className="p-4 border-t border-gray-700 bg-gray-900">
            ...
            <div className="flex justify-between items-center mb-4 pt-2 border-t border-gray-700">
              ...
              <span className="text-white font-extrabold text-2xl">${total.toLocaleString()}</span>
            </div>
            <button
              onClick={onCheckout}
